...
const transporter = nodemailer.createTransport({
  service: 'Outlook365',
  auth: {
    user: 'iventura@proztec.com',         // replace with your email
    pass: 'yqgnhxlpftshlzcc',      // use App Password if 2FA enabled
  },
});
...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    const [managerRows] = await db.query(
      'SELECT name FROM line_managers WHERE manager_id = ?',
      [line_manager_id]
    );
    ...
    // Decide allowed_leave based on user_type
    let allowedLeave = null;
    if (user_type === 'Head Office Employee') {
      allowedLeave = 30;
    } else if (user_type === 'External Employee') {
      allowedLeave = 14;
    }
